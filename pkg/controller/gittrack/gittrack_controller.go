...
func (r *ReconcileGitTrack) newGitTrackObjectInterface(name string, u *unstructured.Unstructured, gitTrackNamespace string) (farosv1alpha1.GitTrackObjectInterface, error) {
...
func (r *ReconcileGitTrack) handleObject(u *unstructured.Unstructured, owner farosv1alpha1.GitTrackInterface) result {
	name := objectName(u)
	gto, err := r.newGitTrackObjectInterface(name, u, owner.GetNamespace())
	if err != nil {
		namespacedName := strings.TrimLeft(fmt.Sprintf("%s/%s", u.GetNamespace(), name), "/")
		return errorResult(namespacedName, err)
	}

	ignored, reason, err := r.ignoreObject(u, owner.GetNamespace())
	if err != nil {
...
func (r *ReconcileGitTrack) ignoreObject(u *unstructured.Unstructured, gitTrackNamespace string) (bool, string, error) {
	...
	if namespaced {
		if !farosflags.AllowCrossNamespaceOwnership && gitTrackNamespace != "" && gitTrackNamespace != u.GetNamespace() {
		...
		if !farosflags.AllowCrossNamespaceOwnership && gitTrackNamespace != "" {
